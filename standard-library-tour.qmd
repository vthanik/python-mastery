...
`Counter` arithmetic merges two counters: `c + c2`, `c - c2`. Missing keys return `0`, never `KeyError`.
...
For anything time-zone-sensitive, always include `tzinfo` (`timezone.utc` for UTC, `zoneinfo.ZoneInfo("Europe/London")` for local zones — Python 3.9+).
