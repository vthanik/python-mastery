...
You'll meet `namedtuple` constantly when reading other people's code: many stdlib functions return them (`os.stat`, `time.localtime`, `urllib.parse.urlparse`). For *new* code that wants type hints or extra methods, prefer `typing.NamedTuple` (a typed namedtuple) or `@dataclass(frozen=True)`. The full comparison is in @sec-dataclasses.
