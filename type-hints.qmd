...
- `class Drawable(Protocol):` declares a *structural* type. Any class with the listed methods (here, `draw(self) -> None`) is automatically a `Drawable` — no `class Circle(Drawable):` inheritance line needed.
- `def draw(self) -> None: ...` is a method *signature* with `...` as the body. It's a placeholder — the protocol only describes the shape, never executes.
- `def render(shape: Drawable) -> None:` accepts any object whose class satisfies the protocol. `Circle` and `Square` both qualify because they happen to define `draw(self) -> None`.
- `@runtime_checkable` is the only reason `isinstance(Circle(), Drawable)` works at runtime — it asks Python to actually inspect attributes when asked. Without it, the protocol is purely a static-checker contract.
