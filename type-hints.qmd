...
A function without hints is still legal. Hints are added when they earn their keep — usually at API boundaries:
...
For collections, since Python 3.9 you can subscript the built-ins directly — no `from typing import List` needed:
...
A *bounded* `TypeVar` restricts the choice of types:
...
Now `average` works on any of the three numeric types but not, say, on `str`.
