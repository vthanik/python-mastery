...
Python 3.11 added `asyncio.TaskGroup` — the modern replacement for `gather` when you want *structured concurrency*. If any task raises, sibling tasks are cancelled and the group re-raises an `ExceptionGroup`. Prefer it for new code.
...
Walking through the structured form:

- `async with asyncio.TaskGroup() as tg:` opens an async context manager — it has `__aenter__` and `__aexit__` instead of `__enter__`/`__exit__`, awaitable versions of the same protocol.
...
- The body of the `async with` exits *after* every owned task completes — the group implicitly awaits its children. No explicit `gather` needed.
- `t1.result()` reads the finished task's return value. Because the group has already awaited the tasks, calling `.result()` is non-blocking here.
- If any task raises, the group cancels the rest and re-raises an `ExceptionGroup` — no orphan tasks running silently after a failure.

The block exits only when every task finishes. Errors compose into an `ExceptionGroup` (see @sec-with-exception-group), which `except*` matches by type.

The general pattern: prefer `TaskGroup` over `gather` for new code. Same concurrency, plus structured cancellation and clean error aggregation.
