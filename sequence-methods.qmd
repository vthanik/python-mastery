...
- `hash(v1) == hash(v2)` because `1 == 1.0` and `hash(1) == hash(1.0)` — Python's numeric tower keeps these aligned. That's the rule "equal objects must have equal hashes" in action.
