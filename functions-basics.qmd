...
## Build: memoizing recursion, then replacing it with a generator {#sec-fn-build}
...
`fib_stream` is an *infinite* generator — the `while True` never exits — but `next` only computes one term at a time, so the loop is harmless. `itertools.islice(gen, 10)` takes the first 10 values, the way `[:10]` would on a list (we cover `itertools` in @sec-stdlib-itertools).
