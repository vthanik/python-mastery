...
To **modify** an outer variable from inside a function, you need an explicit declaration: `global` or `nonlocal`. Without it, Python's "assignments make locals" rule will silently shadow the outer name instead.
...
- `make_counter` defines a local `count = 0`, then a nested function `increment`, then returns the nested function (without calling it).
- Inside `increment`, `nonlocal count` declares "I'm not creating a local — I want to bind to the `count` in the enclosing scope."
- Without that line, `count += 1` would try to read-then-write a local `count` and raise `UnboundLocalError`.
- `make_counter()` returns the `increment` function. Calling `c()` repeatedly increments the *same* `count` because `increment` holds a reference to its enclosing scope's `count`.

The general rule: `nonlocal name` says "modify the enclosing function's `name`"; `global name` says "modify the module-level `name`". The function `increment` is a **closure** — it carries `count` with it after `make_counter` returns.
...
You want to time how long a function takes — and you want it for *several* functions, without copying timing code into each one. A decorator is a function that takes a function and returns a (usually wrapped) function — the natural way to add the same behavior around many call sites. The `@` syntax applies it.
...
- `timer` is the decorator — it takes a function `func` and returns a *new* function `wrapper` that calls `func` with timing around it.
- `wrapper(*args, **kwargs)` accepts any arguments and forwards them to the wrapped function — so the decorator works on any signature.
- `time.perf_counter()` reads a high-resolution timer; the difference gives elapsed seconds.
- `@functools.wraps(func)` copies `func`'s name, docstring, and attributes onto `wrapper` — without it, `slow_sum.__name__` would become `"wrapper"` and tracebacks would lie.
- `@timer` above `def slow_sum(n):` is exactly equivalent to writing `slow_sum = timer(slow_sum)` after the def. The `@` is just sugar for "rebind this name to the decorator's output."
- `slow_sum(1_000_000)` actually calls `wrapper(1_000_000)`, which prints the timing line and returns the real result.
...
Sometimes you want to produce a sequence of values, but materialising the whole list up front is wasteful — or impossible (the sequence is infinite). A function that uses `yield` instead of `return` is a **generator**. It produces values one at a time, on demand, and pauses between them.
...
- The `yield` keyword turns `countdown` into a generator function. Calling `countdown(5)` doesn't run the body — it returns a generator object that *will* run the body, one `yield` at a time, when asked.
- Each `yield n` produces a value to whoever is consuming the generator, then *pauses* the function with all its locals intact.
- When the consumer asks for the next value, the function resumes after the `yield`, runs `n -= 1`, and loops back to `yield n` again.
- `list(countdown(5))` walks the generator to exhaustion, collecting all yielded values into a list.
...
- `countdown(3)` makes a generator but runs none of the body yet.
- Each `next(gen)` resumes the generator until the next `yield`, then returns the yielded value.
- A fourth `next(gen)` would raise `StopIteration` — the generator's signal that it's done.
