...
Catching `Exception` is too broad; catching `ValueError` everywhere a domain error happens is sloppy. The right move is to define your own hierarchy: a base class for "any error from this app" plus a subclass per logical category. Callers handle specific errors by class, never by string-matching.
...
- `class AppError(Exception):` declares an empty subclass of `Exception`. The docstring is the entire body — that's enough for a marker class.
- `DataError` and `AuthError` extend `AppError`, so any `except AppError:` clause catches both.
- `AuthError.__init__` stores extra fields (`user`, `reason`) for programmatic access *and* calls `super().__init__(...)` to set the formatted message that `str(exc)` returns.
- The `try` raises `AuthError`; the `except AppError as e` clause matches because `AuthError` *is-a* `AppError`.

The general rule: subclass `Exception` for an app-specific base, then add a subclass per error category. Callers can catch the base for "any of our errors" or a specific subclass for surgical handling.
...
- `tempfile.mkdtemp()` makes a fresh temp directory; the `Path / "hello.txt"` builds the file path.
- `target.write_text(...)` is a one-line write — sets up the input file for the demo.
