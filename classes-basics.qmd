...
Read this from first principles, one step at a time:
...
3. Why write `cls(...)` instead of `Person(...)` directly? So that subclasses get instances of *themselves*, not of the parent. Watch what happens with a subclass:
...
- `Employee` inherits `from_birth_date` unchanged — we didn't redefine it.
- But when you call `Employee.from_birth_date(...)`, Python now passes `Employee` as `cls`. So inside the method, `cls(name, year)` is `Employee(name, year)` and the result is an `Employee`.
