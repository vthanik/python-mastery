...
- `@classmethod` tells Python "when this method is called, pass the class itself as the first argument." Inside the body, `cls` *is* `Person`.
- `cls(name, year)` calls `Person(name, year)` — i.e., the normal `__init__`. Using `cls` instead of hard-coding `Person` is what makes alternative constructors work for subclasses: if someone defines `class Employee(Person):`, then `Employee.from_birth_date(...)` runs `cls(...)` where `cls` is `Employee`, and the result is an `Employee`. Hard-coding `Person(...)` would be a subtle inheritance bug.
