...
The general rule: `d[key]` is loud (raises on miss); `d.get(key, default)` is quiet (returns the default). Modify with assignment, `del`, or `.pop`:
...
- `[x ** 2 for x in range(10)]` reads "the list of `x ** 2` for each `x` in `range(10)`" — equivalent to a 4-line `for` loop with `.append(x ** 2)`.
