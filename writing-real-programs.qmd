...
- `Mapping[str, float]` is the abstract "anything you can do dict-style lookup on with string keys and float values" — a plain `dict` qualifies, but so does a `defaultdict`, a `ChainMap`, or any custom class implementing the mapping protocol.
- The annotation is a contract: "I'll only call `.values()` and other read-only mapping operations" — narrower than `dict`, more flexible for callers.
