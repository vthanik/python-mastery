...
Without the walrus you'd run the regex twice, or assign on a separate line first. The same pattern collapses a `while` loop that processes input until exhausted:
...
The condition reads "while the next chunk, bound to `chunk`, is truthy." Don't reach for `:=` everywhere — it's earned its place when it removes a duplicate call.
